...
func New() {
	...
	my := fmt.Sprintf("%s:%s@tcp(%s)/%s?charset=utf8&parseTime=True&loc=Local", "root", "qq74263827", "127.0.0.1:3306", "libraryV2")
	conn, err := gorm.Open(mysql.Open(my), &gorm.Config{})
	if err != nil {
		fmt.Printf("err:%s\n", err)
	}
	Conn = conn
	Conn.AutoMigrate(&BookInfo{}, &Category{}, &User{}, &Librarian{}, &Record{})
	MysqlLogger()
}
